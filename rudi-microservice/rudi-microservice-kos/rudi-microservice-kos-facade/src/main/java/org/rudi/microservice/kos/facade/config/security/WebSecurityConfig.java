...
public class WebSecurityConfig {
	...
	@Value("${rudi.kos.security.pre-authentication.disabled:false}")
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		if (!disableAuthentification) {
			http.addFilterAfter(createPreAuthenticationFilter(), BasicAuthenticationFilter.class)
					...
					.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(authorizeHttpReq -> {
			...
			if (!disablePreAuthentification) {
				log.warn("Skos pre-authentication is enabled");
				http.addFilterAfter(createPreAuthenticationFilter(), BasicAuthenticationFilter.class);
			}
		} else {
			log.warn("Skos authentication is disabled");
