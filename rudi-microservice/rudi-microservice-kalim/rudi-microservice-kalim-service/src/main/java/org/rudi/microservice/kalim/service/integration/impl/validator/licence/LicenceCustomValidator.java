...
@Component
public class LicenceCustomValidator implements ElementValidator<LicenceCustom> {
	...
	public Set<IntegrationRequestErrorEntity> validate(LicenceCustom licenceCustom) {
		...
		if (CollectionUtils.isEmpty(licenceCustom.getCustomLicenceLabel())) {
			...
			IntegrationRequestErrorEntity integrationRequestError = new IntegrationRequestErrorEntity(UUID.randomUUID(),
					IntegrationError.ERR_202.getCode(), errorMessage,
					RudiMetadataField.CUSTOM_LICENCE_LABEL.getLocalName(), LocalDateTime.now());
