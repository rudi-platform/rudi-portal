package org.rudi.microservice.kalim.service.integration.impl.validator.authenticated;
...
public class MetadataInfoProviderIsAuthenticatedValidator {
	...
	public Set<IntegrationRequestErrorEntity> validate(Metadata metadata, IntegrationRequestEntity integrationRequest) {
		...
		if (metadataProvider == null) {
			...
			errors.add(new IntegrationRequestErrorEntity(UUID.randomUUID(), ERR_202.getCode(), ERR_202.getMessage(),
					RudiMetadataField.METADATA_INFO_PROVIDER.getName(), LocalDateTime.now()));

		} else {

			final var declaredProviderId = metadataProvider.getOrganizationId();
			if (!authenticatedProviderId.equals(declaredProviderId)) {
				errors.add(new Error303Builder().field(RudiMetadataField.METADATA_INFO_PROVIDER_ORGANIZATION_ID)
						.fieldValue(declaredProviderId.toString())
						.expectedString("l'UUID du nœud fournisseur authentifié").build());
			}
