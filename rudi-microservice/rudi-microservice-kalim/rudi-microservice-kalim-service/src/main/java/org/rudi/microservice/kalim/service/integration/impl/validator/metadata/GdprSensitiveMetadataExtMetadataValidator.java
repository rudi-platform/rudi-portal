...
public class GdprSensitiveMetadataExtMetadataValidator extends AbstractMetadataValidator<Metadata> {
	...
	public Set<IntegrationRequestErrorEntity> validate(Metadata metadata) {
		...
		if (Boolean.TRUE.equals(metadata.getAccessCondition().getConfidentiality().getGdprSensitive())
			...
			IntegrationRequestErrorEntity integrationRequestError = new IntegrationRequestErrorEntity(
					IntegrationError.ERR_107.getCode(), errorMessage);
		...
		if (Boolean.TRUE.equals(metadata.getAccessCondition().getConfidentiality().getGdprSensitive())
				...
				&& metadata.getExtMetadata().getExtSelfdata().getExtSelfdataContent()
						.getSelfdataAccess() == SelfdataContent.SelfdataAccessEnum.API
			...
			IntegrationRequestErrorEntity integrationRequestError = new IntegrationRequestErrorEntity(
					IntegrationError.ERR_305.getCode(), errorMessage);
