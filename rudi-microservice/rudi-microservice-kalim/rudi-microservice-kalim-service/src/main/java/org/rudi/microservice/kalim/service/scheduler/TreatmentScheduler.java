...
public class TreatmentScheduler {
	...
	public void handleIntegrationRequestReport() {
		...
		log.info("Start TreatmentScheduler...");
		...
		if (CollectionUtils.isNotEmpty(integrationRequests.getContent())) {
			log.info("TreatmentScheduler nothing find {}", integrationRequests.getTotalElements());
			for (IntegrationRequest integrationRequest : integrationRequests) {
				try {
					...
					log.info("TreatmentScheduler skip {} - {}", integrationRequest.getUuid(), e.getMessage());
				}
			}
		} else {
		...
		log.info("TreatmentScheduler done.");
	}
}
