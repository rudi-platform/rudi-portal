package org.rudi.microservice.projekt.facade.config.security;
...
public class WebSecurityConfig {
	...
	private static final String[] SB_PERMIT_ALL_URL = {
			// URL public
			"/projekt/v1/application-information", "/projekt/v1/healthCheck",
			// swagger ui / openapi
			"/projekt/v3/api-docs/**", "/projekt/swagger-ui/**", "/projekt/swagger-ui.html",
			"/projekt/swagger-resources/**", "/configuration/ui", "/configuration/security", "/webjars/**" };
	...
	@Value("${rudi.projekt.security.authentication.disabled:false}")
	private boolean disableAuthentification = false;

	@Value("${rudi.projekt.security.pre-authentication.disabled:false}")
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		if (!disableAuthentification) {
			...
			if (!disablePreAuthentification) {
				log.warn("Projekt pre-authentication is enabled");
