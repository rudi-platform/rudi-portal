...
@Repository
public class NewDatasetRequestCustomDaoImpl
		extends AbstractCustomDaoImpl<NewDatasetRequestEntity, NewDatasetRequestSearchCriteria>
		implements NewDatasetRequestCustomDao {
	...
	@Override
	public Page<NewDatasetRequestEntity> searchNewDatasetRequest(NewDatasetRequestSearchCriteria searchCriteria,
			Pageable pageable) {
		return search(searchCriteria, pageable);
	}

	@Override
	protected void addPredicates(NewDatasetRequestSearchCriteria searchCriteria, CriteriaBuilder builder,
			CriteriaQuery<?> criteriaQuery, Root<NewDatasetRequestEntity> root, List<Predicate> predicates) {
		if (StringUtils.isNotEmpty(searchCriteria.getTitleDescription())) {
			predicates.add(builder.or(
					buildPredicateStringCriteria(searchCriteria.getTitleDescription(), "title", false, false, builder,
							root),
					buildPredicateStringCriteria(searchCriteria.getTitleDescription(), "description", false, false,
							builder, root)));
		}
		...
		if (CollectionUtils.isNotEmpty(searchCriteria.getProjectOwnerUuids())) {
			...
			Join<ProjectEntity, NewDatasetRequestEntity> joinDatasetRequest = subRoot
					.join(ProjectEntity.FIELD_DATASET_REQUESTS, JoinType.INNER);
		...
		if (CollectionUtils.isNotEmpty(searchCriteria.getStatus())) {
			List<NewDatasetRequestStatus> statusList = searchCriteria.getStatus().stream()
					.map(status -> getEntityStatus(status.name())).collect(Collectors.toList());
			predicates.add(root.get(NewDatasetRequestEntity.FIELD_NEW_DATASET_REQUEST_STATUS).in(statusList));
		}
	}

	private static org.rudi.microservice.projekt.storage.entity.newdatasetrequest.NewDatasetRequestStatus getEntityStatus(
			String name) {
