package org.rudi.microservice.konsent.facade.config.security;
...
public class WebSecurityConfig {
	...
	private static final String[] SB_PERMIT_ALL_URL = {
			// URL public
			"/konsent/v1/application-information", "/konsent/v1/healthCheck",
			// swagger ui / openapi
			"/konsent/v3/api-docs/**", "/konsent/swagger-ui/**", "/konsent/swagger-ui.html",
			"/konsent/swagger-resources/**", "/configuration/ui", "/configuration/security", "/webjars/**" };
	...
	@Value("${rudi.konsent.security.authentication.disabled:false}")
	private boolean disableAuthentification = false;

	@Value("${rudi.konsent.security.pre-authentication.disabled:false}")
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		if (!disableAuthentification) {
			...
			if (!disablePreAuthentification) {
				log.warn("Konsent pre-authentication is enabled");
