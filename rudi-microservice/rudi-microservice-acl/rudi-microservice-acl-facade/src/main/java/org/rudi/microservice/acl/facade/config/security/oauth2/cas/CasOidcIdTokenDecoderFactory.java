...
import org.rudi.common.facade.config.filter.JwkResourceRetriever;
...
@Slf4j
@Component
public class CasOidcIdTokenDecoderFactory implements JwtDecoderFactory<ClientRegistration> {

	private static final String FAILED_TO_FIND_A_SIGNATURE_VERIFIER_FOR_CLIENT_REGISTRATION = "Failed to find a Signature Verifier for Client Registration: '";
	...
	@Value("${search.lemonng.clockSew:1}")
	private Integer clockSew;

	private Function<ClientRegistration, OAuth2TokenValidator<Jwt>> jwtValidatorFactory = null;
	...
	public JwtDecoder createDecoder(ClientRegistration clientRegistration) {
		...
		if (log.isInfoEnabled()) {
			log.info("createDecoder {}-{} / {} - {}", clientRegistration.getRegistrationId(),
					clientRegistration.getClientId(), clientRegistration.getScopes(),
		...
		return this.jwtDecoders.computeIfAbsent(clientRegistration.getRegistrationId(), key -> {
			...
			jwtDecoder.setJwtValidator(getJwtValidatorFactory().apply(clientRegistration));
	...
	private Function<ClientRegistration, OAuth2TokenValidator<Jwt>> getJwtValidatorFactory() {
		if (jwtValidatorFactory == null) {
			jwtValidatorFactory = new CasOidcIdTokenValidatorFactory(clockSew);
		}
	...
	private NimbusJwtDecoder buildDecoder(ClientRegistration clientRegistration) {
		...
		if (jwsAlgorithm != null && SignatureAlgorithm.class.isAssignableFrom(jwsAlgorithm.getClass())) {
			...
			if (!StringUtils.hasText(jwkSetUri)) {
				OAuth2Error oauth2Error = new OAuth2Error(MISSING_SIGNATURE_VERIFIER_ERROR_CODE,
						FAILED_TO_FIND_A_SIGNATURE_VERIFIER_FOR_CLIENT_REGISTRATION
								+ clientRegistration.getRegistrationId()
								+ "'. Check to ensure you have configured the JwkSet URI.",
						null);
				throw new OAuth2AuthenticationException(oauth2Error, oauth2Error.toString());
			}
			return buildNimbusDecoder(jwkSetUri, (SignatureAlgorithm) jwsAlgorithm, false);
		}
		...
		OAuth2Error oauth2Error = new OAuth2Error(MISSING_SIGNATURE_VERIFIER_ERROR_CODE,
				FAILED_TO_FIND_A_SIGNATURE_VERIFIER_FOR_CLIENT_REGISTRATION + clientRegistration.getRegistrationId()
						+ "'. Check to ensure you have configured a valid JWS Algorithm: '" + jwsAlgorithm + "'",
				null);
		throw new OAuth2AuthenticationException(oauth2Error, oauth2Error.toString());
	}

	protected NimbusJwtDecoder buildNimbusDecoder(String jwkSetUri, SignatureAlgorithm jwsAlgorithm,
			boolean hostVerifier) {
		return new NimbusJwtDecoder(processor(jwkSetUri, jwsAlgorithm, hostVerifier));
	}

	/**
	 * 
	 * @param jwkSource    la source
	 * @param jwsAlgorithm l'algorithme
	 * @return
	 */
	protected JWSKeySelector<SecurityContext> jwsKeySelector(JWKSource<SecurityContext> jwkSource,
			SignatureAlgorithm jwsAlgorithm) {
		return new JWSVerificationKeySelector<>(JWSAlgorithm.RS256, jwkSource);
	}

	protected JWKSource<SecurityContext> jwkSource(ResourceRetriever jwkSetRetriever, String jwkSetUri) {
		return JWKSourceBuilder.create(toURL(jwkSetUri), jwkSetRetriever).build();
	}
	...
	protected JWTProcessor<SecurityContext> processor(String jwkSetUri, SignatureAlgorithm jwsAlgorithm,
			boolean hostVerifier) {
		ResourceRetriever jwkSetRetriever = new JwkResourceRetriever(JWKSourceBuilder.DEFAULT_HTTP_CONNECT_TIMEOUT,
				JWKSourceBuilder.DEFAULT_HTTP_READ_TIMEOUT, JWKSourceBuilder.DEFAULT_HTTP_SIZE_LIMIT, true,
				hostVerifier);
		...
		jwtProcessor.setJWSKeySelector(jwsKeySelector(jwkSource, jwsAlgorithm));

		return jwtProcessor;
	}

	/**
	 * Sets the factory that provides an {@link OAuth2TokenValidator}, which is used by the {@link JwtDecoder}. The default composes
	...
	/**
	 * Sets the resolver that provides the expected {@link JwsAlgorithm JWS algorithm} used for the signature or MAC on the {@link OidcIdToken ID Token}.
	 * The default resolves to {@link SignatureAlgorithm#RS256 RS256} for all {@link ClientRegistration clients}.
	 * 
	 * @param jwsAlgorithmResolver the resolver that provides the expected {@link JwsAlgorithm JWS algorithm} for a specific {@link ClientRegistration
	 *                             client}
	 */
	...
	/**
	 * Sets the factory that provides a {@link Converter} used for type conversion of claim values for an {@link OidcIdToken}. The default is
