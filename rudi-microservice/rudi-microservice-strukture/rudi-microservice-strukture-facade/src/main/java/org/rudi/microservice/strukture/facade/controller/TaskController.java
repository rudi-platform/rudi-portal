...
public class TaskController implements TasksApi {
	...
	private final TaskQueryService<StruktureTaskSearchCriteria> taskQueryService;

	/**
	 * GET /tasks/{task-id} : Chargement d&#39;une tâche
	 ...
	 * or Request Timeout (status code 408)
	...
	public ResponseEntity<List<Task>> searchTasks(
			...
			List<Status> status,
			List<String> fonctionalStatus,
			OrganizationStatus organizationStatus,
			Boolean asAdmin,
			UUID organizationUuid) throws Exception {

		StruktureTaskSearchCriteria criteria = StruktureTaskSearchCriteria
				.builder()
				.name(name)
