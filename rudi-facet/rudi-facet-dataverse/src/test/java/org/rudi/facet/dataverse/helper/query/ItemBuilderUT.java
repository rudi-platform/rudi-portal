...
class ItemBuilderUT {
	...
	private static final FieldSpec FIELD = ROOT.newChildFromJavaField("field").isDirectSortable(false);

	/**
	 * RUDI-961 On échappe les caractères spéciaux Solr Source :
	...
	/**
	 * RUDI-961 On n'échappe pas les caractères non spéciaux
	 */
	@ParameterizedTest(name = "{0}")
	@CsvSource(value = { "&,&", "|,|", "\\,\\" })
	...
	/**
	 * RUDI-961 On n'échappe pas certains caractères spéciaux lorsqu'ils sont utilisés seuls
	 */
	@ParameterizedTest(name = "{0}")
	@CsvSource(value = { "*,*", })
