...
class MediaSearchCriteriaMapperUT {
	...
	void mediaSearchCriteriaToSearchParams() {
		final MediaSearchCriteria mediaSearchCriteria = new MediaSearchCriteria()
				...
				.mediaAuthorAffiliation(MediaOrigin.PROVIDER).limit(12).offset(24).kindOfData(KindOfData.LOGO)
				.order("asc");

		final SearchParams searchParams = mapper.mediaSearchCriteriaToSearchParams(mediaSearchCriteria);

		assertThat(searchParams).hasFieldOrPropertyWithValue("type", EnumSet.of(SearchType.DATASET))
				.hasFieldOrPropertyWithValue("q",
						"(authorIdentifier:\"5596b5b2\\-b227\\-4c74\\-a9a1\\-719e7c1008c7\") AND (authorAffiliation:\"provider\") AND (kindOfData:\"LOGO\")")
				.hasFieldOrPropertyWithValue("subtree", mediaDataAlias).hasFieldOrPropertyWithValue("sortBy", "name")
				.hasFieldOrPropertyWithValue("sortOrder", "asc").hasFieldOrPropertyWithValue("perPage", 12)
				.hasFieldOrPropertyWithValue("start", 24).hasFieldOrPropertyWithValue("showRelevance", null)
